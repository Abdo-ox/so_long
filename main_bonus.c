...
/*   Updated: 2023/01/27 08:05:19 by ajari            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_hook(int k, t_mx *m)
{
	...
	if (!m->stop)
	{
		...
		if ((k == 126 || k == 13) && is_allow(m, m->x, m->y - 1))
			move_up(m);
		if (m->n_c_token == m->n_of_c / 2)
			put_image_to_w(*m, m->image.d_s, m->d_x * m->w, m->d_y * m->h);
		if (m->n_of_c == m->n_c_token && !m->stop)
			put_image_to_w(*m, m->image.d_o, m->d_x * m->w, m->d_y * m->h);
	}
	return (0);
}
...
void	make_window(t_mx *m)
{
	...
	m->x_w = mlx_new_window(m->mx, x * 60, i * 60, "./so_long");
	m->w_w = (x * 30 - 300);
	m->h_w = (i * 30 - 75);
	fill_im(m, 0, 0);
	...
	mlx_loop_hook(m->mx, animation, m);
	mlx_loop(m->mx);
}
